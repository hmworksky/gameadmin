...
from apps.ApiManager import separator
from apps.ApiManager.models import ProjectInfo, ModuleInfo, TestCaseInfo, UserInfo, EnvInfo, TestReports, DebugTalk, \
    TestSuite
from apps.ApiManager.tasks import main_hrun
from apps.ApiManager.utils.common import module_info_logic, project_info_logic, case_info_logic, config_info_logic, \
...
from apps.ApiManager.utils.operation import env_data_logic, del_module_data, del_project_data, del_test_data, copy_test_data, \
    del_report_data, add_suite_data, copy_suite_data, del_suite_data, edit_suite_data
from apps.ApiManager.utils.pagination import get_pager_info
from apps.ApiManager.utils.runner import run_by_batch, run_test_by_type, NewRunner, switch_session
from apps.ApiManager.utils.task_opt import delete_task, change_task_status
from apps.ApiManager.utils.testcase import get_time_stamp
...
def level_check(level):
    def outer(func):
        def wrapper(request, *args, **kwargs):
            username = request.session.get('now_account')
            user_level = UserInfo.objects.filter(username=username).values("level")
            if user_level < level:
                pass
                # return HttpResponseRedirect('/api/login/')
            return func(request, *args, **kwargs)
...
def run_batch_test(request):
    ...
    if request.is_ajax():
        ...
        method = "PRIMUS" if base_url.startswith("ws") else ""
        session = switch_session(method, base_url)
        runner.run(testcase_dir_path, http_client_session=session)
