...
from apps.ApiManager.models import ModuleInfo, TestCaseInfo, TestReports, TestSuite
from apps.ApiManager.utils.operation import add_project_data, add_module_data, add_case_data, add_config_data, \
    add_register_data
from apps.ApiManager.utils.task_opt import create_task
...
class DB:
    ...
    def select(self, where, values, limit=50, order_by_data=None, group_by_data=None, is_desc=True):
        ...
        if order_by_data:
            data = data.order_by(order_by_data)
        if not is_desc:
...
class MyRedis:
    import sys
    from apps.ApiManager.utils import config
